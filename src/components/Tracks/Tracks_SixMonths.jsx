...
export default function TracksLast6Months(data){    
    
    const elements = Object.values(data.user_data["six_months_tracks"]).map((track,i) => {
        return (
    ...
    return (
        <div>
            {elements ? <PopUpTrack artist = {data.user_data["six_months_tracks"][0]}/>: null}
            {elements}
        </div>
    )
    }