...
export default function TracksLastMonth(data){
    const elements = Object.values(data.user_data["last_month_tracks"]).map((track,i) => {
        return (
    ...
    return (
        <div>
            {elements ? <PopUpTrack artist = {data.user_data["last_month_tracks"][0]}/>: null}
            {elements}
        </div>
    )
    }